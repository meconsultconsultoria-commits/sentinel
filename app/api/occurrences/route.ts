...
const mapOccurrence = (r:any) => ({
  id:r.id, createdAt:r.created_at, date:r.date||"", time:r.time||"", company:r.company_id||"",
  unit:r.unit_id||"", operation:r.operation||"", location:r.location||"", city:r.city||"", uf:r.uf||"",
...
export async function GET(req:Request){
  ...
  if(id){
    const row=await env.DB.prepare("SELECT * FROM occurrences WHERE id=?").bind(id).first();
  ...
  const rows=await env.DB.prepare("SELECT * FROM occurrences ORDER BY created_at DESC").all();
